...
namespace RoadCreatorPro
{
    ...
    public class ProhibitedAreaEditor : Editor
    {
        ...
        public void OnSceneGUI()
        {
            ...
            if (PrefabStageUtility.GetPrefabStage(area.gameObject) != null)
            {
                return;
            }
        ...
        private void GetCurrentPointIndex(ProhibitedArea area)
        {
            ...
            if (currentMovingPointIndex == -1 && GUIUtility.hotControl == 0 && area.handleIds.Contains(HandleUtility.nearestControl))
            {
                ...
                if (EditPointWindow.instance != null)
                {
                    ...
                    editPointWindow.lastPoint = area.transform.GetChild(0).GetChild(lastPointIndex / 3).GetComponent<Point>();
                    editPointWindow.lastPointIndex = 0;
                }
            }
        }
        ...
        {
            if (PrefabStageUtility.GetPrefabStage(((ProhibitedArea)target).gameObject) != null)
            {
                return;
            }
