        ...
        public static SettingsProvider CreateSettingsProvider()
        {
            SettingsProvider settingsProvider = new SettingsProvider("Project/RoadCreator", SettingsScope.Project)
            {
                ...
                guiHandler = (searchContext) =>
                {
                    ...
                    EditorGUILayout.PropertyField(settings.FindProperty("sceneViewMenu"));
                    if (settings.FindProperty("sceneViewMenu").enumValueIndex != 0)
                    {
                        settings.FindProperty("showSceneViewMenuForAllObjects").boolValue = EditorGUILayout.Toggle(new GUIContent("Show Scene View Menu For All Object", "If disabled then it is only showed when a road, prefab line, intersection or prohibited area is selected"), settings.FindProperty("showSceneViewMenuForAllObjects").boolValue);
                    }
                    ...
                    settings.FindProperty("intersectionCurvePointSize").floatValue = Mathf.Clamp(EditorGUILayout.FloatField("Intersection Curve Point Size", settings.FindProperty("intersectionCurvePointSize").floatValue), 0.001f, 30f);
                    ...
                    {
                        settings.FindProperty("sceneViewMenu").enumValueIndex = 3;
                        settings.FindProperty("showSceneViewMenuForAllObjects").boolValue = false;
                        settings.FindProperty("anchorPointSize").floatValue = 1.5f;
                        settings.FindProperty("controlPointSize").floatValue = 1.5f;
                        settings.FindProperty("intersectionCurvePointSize").floatValue = 0.3f;
