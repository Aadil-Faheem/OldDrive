...
namespace RoadCreatorPro
{
    public class RoadCreator : PointSystemCreator
    {
            ...
            if (transform.GetChild(0).childCount > 1)
            {
                ...
                if (CheckVariables(pointData))
                {
                    ...
                    if (startIntersection != null)
                    {
                        ...
                        {
                            RoadCreator otherRoad = startIntersection.connections[i].roadPoint.transform.parent.parent.GetComponent<RoadCreator>();
                    ...
                    if (endIntersection != null)
                    {
                        ...
                        {
                            RoadCreator otherRoad = endIntersection.connections[i].roadPoint.transform.parent.parent.GetComponent<RoadCreator>();
        ...
        private void GenerateGuidelines(PointData pointData)
        {
            for (int i = 0; i < transform.GetChild(0).childCount; i++)
            {
                GenerateGuideline(pointData, i);
            }
        }

        private void GenerateGuideline(PointData pointData, int i)
        {
            // Prevent error
            if (settings == null)
            {
            ...
            Point point = transform.GetChild(0).GetChild(i).GetComponent<Point>();
            Vector3 forward;

            // Don't generate first point if cyclic
            if (cyclic && i == 0)
            {
                point.startGuidelinePoint = Vector3.zero;
                point.endGuidelinePoint = Vector3.zero;
                return;
            }

            if (i < transform.GetChild(0).childCount - 1)
            {
                forward = pointData.positions[pointData.segmentStartIndexes[i] + 2] - pointData.positions[pointData.segmentStartIndexes[i]];
            }
            else
            {
                forward = pointData.positions[pointData.positions.Count - 1] - pointData.positions[pointData.positions.Count - 3];
            }

            Vector3 left = new Vector3(-forward.z, 0, forward.x).normalized;
            point.startGuidelinePoint = left * settings.FindProperty("roadGuidelinesLength").floatValue;
            point.endGuidelinePoint = -left * settings.FindProperty("roadGuidelinesLength").floatValue;
        }
        ...
        private void CreateAndRemoveMeshes()
        {
            ...
            while (transform.GetChild(1).childCount < lanes.Count)
            {
                ...
                mesh.AddComponent<SelectParent>();
        ...
        private void AssignMeshes()
        {
            for (int i = 0; i < lanes.Count; i++)
            {
                ...
                {
                    if (transform.GetChild(1).GetChild(i).GetComponent<MeshCollider>().enabled == false)
                    {
                        transform.GetChild(1).GetChild(i).GetComponent<MeshCollider>().enabled = true;
                    }
                ...
                {
                    if (transform.GetChild(1).GetChild(i).GetComponent<MeshCollider>().enabled == true)
                    {
                        transform.GetChild(1).GetChild(i).GetComponent<MeshCollider>().enabled = false;
                    }
                }
        ...
        private void GenerateMesh(int laneIndex, Vector3 point, Vector3 lastPoint, Vector3 nextPoint, Vector3 forward, float currentDistance, float currentWidth, float currentWidthOffset, float previousYOffset, float currentYOffset, float progress, int pointIndex, ref List<Vector3> lastLeftPoints, ref List<Vector3> lastRightPoints)
        {
            ...
            {
                // Add triangles
                meshTriangles[laneIndex][0].Add(meshVertices[laneIndex][0].Count - 1);
                meshTriangles[laneIndex][0].Add(meshVertices[laneIndex][0].Count - 3);
                meshTriangles[laneIndex][0].Add(meshVertices[laneIndex][0].Count - 2);

                meshTriangles[laneIndex][0].Add(meshVertices[laneIndex][0].Count - 2);
                meshTriangles[laneIndex][0].Add(meshVertices[laneIndex][0].Count - 3);
                meshTriangles[laneIndex][0].Add(meshVertices[laneIndex][0].Count - 4);
            }
            ...
            for (int i = 0; i < lodLevels; i++)
            {
                ...
                if (progress == 0 || progress == 1 || pointIndex % lodVertexDivisions[i] == 0 || point == lastPoint || point == nextPoint)
                {
                    ...
                    {
                        meshTriangles[laneIndex][i + 1].Add(meshVertices[laneIndex][i + 1].Count - 1);
                        meshTriangles[laneIndex][i + 1].Add(meshVertices[laneIndex][i + 1].Count - 3);
                        meshTriangles[laneIndex][i + 1].Add(meshVertices[laneIndex][i + 1].Count - 2);

                        meshTriangles[laneIndex][i + 1].Add(meshVertices[laneIndex][i + 1].Count - 2);
                        meshTriangles[laneIndex][i + 1].Add(meshVertices[laneIndex][i + 1].Count - 3);
                        meshTriangles[laneIndex][i + 1].Add(meshVertices[laneIndex][i + 1].Count - 4);
                    }
                }
            }
        }
        ...
        {
            // Only road points not prefab line points
            RoadCreator[] roads = FindObjectsOfType<RoadCreator>();
            List<Point> points = new List<Point>();

            for (int i = 0; i < roads.Length; i++)
            {
                for (int j = 0; j < roads[i].transform.GetChild(0).childCount; j++)
                {
                    points.Add(roads[i].transform.GetChild(0).GetChild(j).GetComponent<Point>());
                }
            }

            roadPoints = points.ToArray();
        }
        ...
        public PointData CalculatePoints()
        {
            ...
            {
                float curveLength = GetCurveLenth(i, false);
                ...
                // Add last point
                Vector3 lastPoint = Utility.Lerp4(transform.GetChild(0).GetChild(i).position, transform.GetChild(0).GetChild(i + 1).position, transform.GetChild(0).GetChild(i).position + transform.GetChild(0).GetChild(i).GetComponent<Point>().rightLocalControlPointPosition, transform.GetChild(0).GetChild(i + 1).position + transform.GetChild(0).GetChild(i + 1).GetComponent<Point>().leftLocalControlPointPosition, 1);
            ...
            if (connectToIntersections && ((index == 0 && startIntersection == null) || (index == transform.GetChild(0).childCount - 1 && endIntersection == null)))
            {
                ...
                if (Physics.Raycast(movedPoint.transform.position + Vector3.up, Vector3.down, out raycastHit, 10, LayerMask.GetMask("Road")))
                {
                    ...
                    if (nearbyPoint.transform.GetSiblingIndex() > 0 && nearbyPoint.transform.GetSiblingIndex() < nearbyPoint.transform.parent.childCount - 1)
                    {
                        ...
                        extraPoint = nearbyPoint.transform.parent.parent.GetComponent<RoadCreator>().SplitSegment(nearbyPoint.transform.GetSiblingIndex());
                    }
        ...
        private void CreateIntersection(Point movedPoint, Point nearbyPoint, Point extraPoint, Vector3 intersectionPosition)
        {
        ...
        private void ConnectToIntersection(Point movedPoint, Intersection intersection)
        {
            ...
            movedPoint.transform.position = intersection.transform.position - tangent * 6;
        ...
        private void DisconnectFromIntersection(Point movedPoint)
        {
            Undo.RegisterCompleteObjectUndo(this, "Moved Point");
            if (movedPoint.transform.GetSiblingIndex() == 0)
            {
