    ...
    public class ProhibitedArea : MonoBehaviour
    {
        ...
        public List<Vector3> GenerateOuterMesh()
        {
            ...
            for (int i = 0; i < transform.GetChild(0).childCount; i++)
            {
                ...
                Vector3 left = (nextForward - currentForward).normalized;
                left = new Vector3(-left.z, 0, left.x);
                left.y = 0;

                Vector3 innerVertex = outerVertex + left * (outerLineWidth * (1 + (180.1f - Vector3.Angle(nextForward, currentForward)) / 180)); // Compesate for the fact that sharp angles make the line thinner
                ...
                // Triangles
                triangles.Add((vertices.Count - 1) % (transform.GetChild(0).childCount * 2));
                triangles.Add((vertices.Count) % (transform.GetChild(0).childCount * 2));
                triangles.Add((vertices.Count - 2) % (transform.GetChild(0).childCount * 2));
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                for (int j = 0; j < verticesLeft.Count; j++)
                {
                    ...
                    if (!Utility.IsConvex(vertices[verticesLeft[previousVertexIndex]], vertices[verticesLeft[j]], vertices[verticesLeft[nextVertexIndex]]))
                    {
                        ...
                        {
                            // Add triangle
                            triangles.Add(verticesLeft[previousVertexIndex]);
                            triangles.Add(verticesLeft[nextVertexIndex]);
                            triangles.Add(verticesLeft[j]);
        ...
        public void InitializeSystem()
        {
            ...
            if (transform.childCount == 0)
            {
                ...
                mainMesh.AddComponent<SelectParent>();
                ...
                borderMesh.AddComponent<MeshFilter>();
                borderMesh.AddComponent<MeshRenderer>();
                borderMesh.AddComponent<SelectParent>();
