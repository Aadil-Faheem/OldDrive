...
namespace RoadCreatorPro
{
    ...
    public class Intersection : MonoBehaviour
    {
        ...
        public bool automaticallyCalculateCurvePoints = true;
        ...
        public void Regenerate(bool selfChanged, bool updateTerrain = false, bool updateDetails = false, bool updateTrees = false)
        {
            ...
            {
                if (connections[i].roadPoint == null)
                {
                    connections.RemoveAt(i);
                }
            }

            // Remove if less than two connections
            if (connections.Count < 2)
            {
                if (connections.Count == 1)
                {
                    RoadCreator road = connections[0].roadPoint.transform.parent.parent.GetComponent<RoadCreator>();
            ...
            if (selfChanged)
            {
                ...
                for (int i = 0; i < connections.Count; i++)
                {
                    ...
                    {
                        connections[i].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().endIntersectionConnection = connections[i];
                    }
                    else
                    {
                        connections[i].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().startIntersectionConnection = connections[i];
                    }
                }
            }
            else
            {
                ...
                for (int i = 0; i < connections.Count; i++)
                {
                    if (connections[i].endConnection)
                    {
                        connections[i] = connections[i].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().endIntersectionConnection;
                    }
                    else
                    {
                        connections[i] = connections[i].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().startIntersectionConnection;
                    }
                }
            }

            // Set new center position
            Vector3 totalPostion = Vector3.zero;
            for (int i = 0; i < connections.Count; i++)
            {
                totalPostion += connections[i].roadPoint.transform.position;
            }
            transform.position = totalPostion / connections.Count;
            ...
            if (automaticallyCalculateCurvePoints)
            {
                RecalculateTangents();
            }
        ...
        public void RecalculateTangents()
        {
            for (int i = 0; i < connections.Count; i++)
            {
                ...
                PointData pointData = connections[i].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().GetPointData();
                ...
                pointData = connections[nextIndex].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().GetPointData();
                ...
                else
                {
                    ...
                    connections[i].leftTangent = ((connections[i].leftPoint + 2 * intersectPoint) / 3) - connections[i].leftPoint;
                    connections[nextIndex].rightTangent = ((connections[nextIndex].rightPoint + 2 * intersectPoint) / 3) - connections[nextIndex].rightPoint;
                }
            }
        }
        ...
        private void RemoveOldTurnMarkings()
        {
        ...
        private void GenerateMeshSection(int index, ref List<List<Vector3>> vertices, ref List<List<Vector2>> uvs, ref List<List<int>> triangles, Terrain terrain, ref float[,] heightMap, float[,] originalHeightmap, ref HashSet<Vector2Int> terrainPoints, ref HashSet<Vector2Int> finishedTerrainPoints, ref HashSet<Vector2Int> detailHeights, ref Dictionary<Vector2Int, float> terrainDistances, ref List<Vector3> treePositions, ref HashSet<Vector3> treesToRemove, bool updateTerrain, bool updateDetails, bool updateTrees)
        {
            ...
            float segments = Mathf.Max((connections[index].length + connections[nextIndex].length) * detailLevel * 0.3f, 3);
            for (float t = 0; t <= 1 + 1 / segments; t += 1 / segments)
            {
                ...
                // Main lod mesh
                Vector3 outerPoint = Utility.Lerp4(startPoint, endPoint, startTangent, endTangent, realT);
                ...
                if (flipUvs)
                {
                    uvs[0].Add(new Vector2(Vector3.Distance(outerPoint, innerPoint) / uvXScale / 3, t * 3));
                    uvs[0].Add(new Vector2(0, t));
                }
                else
                {
                    uvs[0].Add(new Vector2(0, t));
        ...
        /*private void RaiseSideTerrain(float t, Vector3 outerPoint, Vector3 innerPoint, Vector3 lastOuterPoint, Vector3 lastInnerPoint, Terrain terrain, float[,] heightMap, ref Dictionary<Vector2Int, float> terrainHeights, ref Dictionary<Vector2Int, float> terrainDistances)
        {
            if (t > 0)
            {
                Vector3 lastLeft = (outerPoint - innerPoint).normalized;
                Vector3 left = (outerPoint - innerPoint).normalized;
                float verticalTerrainPointsPercentage = 1f / Mathf.Max(4 * terrainAccuracy, Vector3.Distance(outerPoint, innerPoint) * 8 * terrainAccuracy);
                float horizontalTerrainPointsPercentage = 1f / Mathf.Max(4 * terrainAccuracy, Vector3.Distance(lastOuterPoint + lastLeft * terrainExtraMaxHeight, outerPoint + left * terrainExtraMaxHeight) * 8 * terrainAccuracy);

                for (float z = 0; z < 1; z += horizontalTerrainPointsPercentage)
                {
                    // Flat
                    for (float x = 0; x < 1; x += verticalTerrainPointsPercentage)
                    {
                        Vector3 position = Vector3.Lerp(Vector3.Lerp(lastInnerPoint, innerPoint, z), Vector3.Lerp(lastOuterPoint + lastLeft * terrainExtraMaxHeight, outerPoint + left * terrainExtraMaxHeight, z), x);

                        // World space to local terrain space
                        float terrainX = (position.x - terrain.transform.position.x) / terrain.terrainData.size.x * terrain.terrainData.heightmapResolution;
                        float terrainY = (position.y - terrain.transform.position.y - terrainModificationYOffset) / terrain.terrainData.size.y;
                        float terrainZ = (position.z - terrain.transform.position.z) / terrain.terrainData.size.z * terrain.terrainData.heightmapResolution;
                        int roundedTerrainX = Mathf.RoundToInt(terrainX);
                        int roundedTerrainZ = Mathf.RoundToInt(terrainZ);

                        if (terrainX > 0 && terrainZ > 0 && terrainX < terrain.terrainData.heightmapResolution && terrainZ < terrain.terrainData.heightmapResolution)
                        {
                            //Utility.ModifyHeight(heightMap, ref terrainHeights, ref terrainDistances, terrainX, terrainY, terrainZ, roundedTerrainX, roundedTerrainZ, true, true);
                        }
                    }

                    // Smooth hill to neighboring terrain
                    Vector3 targetPosition = Vector3.Lerp(lastOuterPoint, outerPoint, z) + Vector3.Lerp(lastLeft, left, z) * (terrainExtraMaxHeight + terrainRadius + 1);
                    RaycastHit raycastHit;
                    if (terrain.GetComponent<TerrainCollider>().Raycast(new Ray(targetPosition + Vector3.up * 50, Vector3.down), out raycastHit, 100))
                    {
                    ...
                    for (float x = 0; x < terrainRadius; x += verticalTerrainPointsPercentage)
                    {
                        Vector3 position = Vector3.Lerp(Vector3.Lerp(lastOuterPoint, outerPoint, z) + Vector3.Lerp(lastLeft, left, z) * terrainExtraMaxHeight, targetPosition, x);
                        ...
                        int roundedTerrainX = Mathf.RoundToInt(terrainX);
                        int roundedTerrainZ = Mathf.RoundToInt(terrainZ);
                        float globalX = ((float)roundedTerrainX / terrain.terrainData.heightmapResolution * terrain.terrainData.size.x) + terrain.transform.position.x;
                        float globalZ = ((float)roundedTerrainZ / terrain.terrainData.heightmapResolution * terrain.terrainData.size.z) + terrain.transform.position.z;

                        Vector3 startPosition = Vector3.Lerp(lastOuterPoint, outerPoint, z) + Vector3.Lerp(lastLeft, left, z) * terrainExtraMaxHeight;
                        float actualDistanceX = Vector2.Distance(new Vector2(startPosition.x, startPosition.z), new Vector2(globalX, globalZ)); // Distance original lerp pos to exact pos
                        float defaultDistance = Vector2.Distance(new Vector2(startPosition.x, startPosition.z), new Vector2(targetPosition.x, targetPosition.z));
                        Vector2 newDirection = (new Vector2(globalX, globalZ) - new Vector2(startPosition.x, startPosition.z)).normalized;
                        float angle = Vector2.Angle(new Vector2((targetPosition - startPosition).normalized.x, (targetPosition - startPosition).normalized.z), newDirection);
                        float actualDistanceZ = Mathf.Cos(Mathf.Deg2Rad * angle) * actualDistanceX; // Cosine (angle) * hypotenuse = nearby

                        float positionY = 0;
                        if (actualDistanceZ <= 0)
                        {
                            positionY = startPosition.y;
                        }
                        else if (targetPosition.y > startPosition.y && Mathf.Abs(targetPosition.y - startPosition.y) > 0.01f)
                        {
                            positionY = startPosition.y + actualDistanceZ / Mathf.Tan(Mathf.Deg2Rad * terrainAngle); // Far / Tangent (angle) = near 
                        }
                        else
                        {
                            positionY = startPosition.y - actualDistanceZ / Mathf.Tan(Mathf.Deg2Rad * terrainAngle); // Far / Tangent (angle) = near 
                        ...
                        float terrainY = (positionY - terrain.transform.position.y - terrainModificationYOffset) / terrain.terrainData.size.y;

                        if (terrainX > 0 && terrainZ > 0 && terrainX < terrain.terrainData.heightmapResolution && terrainZ < terrain.terrainData.heightmapResolution)
                        {
                            //Utility.ModifyHeight(heightMap, ref terrainHeights, ref terrainDistances, terrainX, terrainY, terrainZ, roundedTerrainX, roundedTerrainZ, targetPosition.y > startPosition.y && Mathf.Abs(targetPosition.y - startPosition.y) > 0.01f, false);
                        }
                    }
                }
            }
        ...
        */
        private void GenerateMainRoad(int index)
        {
            ...
            PointData pointData = connections[mainRoad.startIndex].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().GetPointData();
            if (connections[mainRoad.startIndex].endConnection)
            {
            ...
            pointData = connections[mainRoad.endIndex].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().GetPointData();
            if (connections[mainRoad.endIndex].endConnection)
            {
            ...
            Vector3 leftTopTangent = leftForward;
            Vector3 rightTopTangent = rightForward;
            Vector3 leftBottomTangent = leftForward;
            Vector3 rightBottomTangent = rightForward;
            ...
            {
                leftTopTangent = (startLeftPosition + 2 * curvePoint) / 3;
                rightTopTangent = (endRightPosition + 2 * curvePoint) / 3;
            }
            else
            {
            ...
            if (curvePoint != Utility.MaxVector3)
            {
                leftBottomTangent = (startRightPosition + 2 * curvePoint) / 3;
                rightBottomTangent = (endLeftPosition + 2 * curvePoint) / 3;
            }
            else
            {
            ...
            for (float t = 0; t <= 1 + 1 / segments; t += 1 / segments)
            {
                ...
                // Main lod level
                // Add vertices
                vertices[0].Add(Utility.Lerp4(startLeftPosition, endRightPosition, leftTopTangent, rightTopTangent, realT) + new Vector3(0, mainRoad.yOffset, 0) - transform.position);
                vertices[0].Add(Utility.Lerp4(startRightPosition, endLeftPosition, leftBottomTangent, rightBottomTangent, realT) + new Vector3(0, mainRoad.yOffset, 0) - transform.position);

                if (mainRoad.flipUvs)
                {
                    uvs[0].Add(new Vector2(1, realT * mainRoad.uvZScale * centerDistance / 3 + mainRoad.uvZOffset));
                    uvs[0].Add(new Vector2(0, realT * mainRoad.uvZScale * centerDistance / 3 + mainRoad.uvZOffset));
                }
                else
                {
                    uvs[0].Add(new Vector2(0, realT * mainRoad.uvZScale * centerDistance / 3 + mainRoad.uvZOffset));
        ...
        private void PlaceTurnMarkings(int index)
        {
            if (connections[index].turnMarkingsRepetitions > 0 && connections.Count > 2)
            {
                ...
                Vector3 startTangent = startPoint + connections[index].roadPoint.GetComponent<Point>().rightLocalControlPointPosition;
                Vector3 endPoint = connections[index].roadPoint.transform.parent.GetChild(1).position;
                Vector3 endTangent = endPoint + connections[index].roadPoint.transform.parent.GetChild(1).GetComponent<Point>().leftLocalControlPointPosition;

                if (connections[index].endConnection)
                {
                    startPoint = connections[index].roadPoint.transform.position;
                    startTangent = startPoint + connections[index].roadPoint.transform.GetComponent<Point>().leftLocalControlPointPosition;
                    ...
                    endTangent = endPoint + connections[index].roadPoint.transform.parent.GetChild(connections[index].roadPoint.transform.parent.childCount - 2).GetComponent<Point>().rightLocalControlPointPosition;
                }
        ...
        private void AssignMesh(List<List<Vector3>> vertices, List<List<Vector2>> uvs, List<List<int>> triangles, string name, List<Material> materials, PhysicMaterial physicsMaterial)
        {
            ...
            meshObject.AddComponent<SelectParent>();
            ...
            while (lodLevels > meshObject.transform.childCount)
            {
                ...
                lodObject.AddComponent<SelectParent>();
            }
        ...
        public void Flatten()
        {
            ...
            for (int i = 0; i < connections.Count; i++)
            {
                centerPoint += connections[i].roadPoint.transform.position.y;
            }
            ...
            for (int i = 0; i < connections.Count; i++)
            {
                ...
                position.y = centerPoint;
                connections[i].roadPoint.transform.position = position;
            }

            for (int i = 0; i < connections.Count; i++)
            {
                connections[i].roadPoint.transform.parent.parent.GetComponent<RoadCreator>().Regenerate(false);
            }
        }
    }
}
