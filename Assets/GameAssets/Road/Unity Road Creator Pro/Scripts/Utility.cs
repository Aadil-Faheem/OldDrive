...
namespace RoadCreatorPro
{
    public class Utility
    {
        ...
        public static Vector3 DrawPositionHandle(bool alwaysScale, float handleSize, Vector3 position, Quaternion rotation)
        {
            ...
            position = Handles.FreeMoveHandle(position, rotation, handleSize * 0.15f, EditorSnapSettings.move, Handles.RectangleHandleCap);
            Handles.color = color;
            return position;
        }
#else
    public static Vector3 DrawPositionHandle(bool alwaysScale, float handleSize, Vector3 position, Quaternion rotation)
    {
        ...
        Handles.color = Handles.centerColor;
        position = Handles.FreeMoveHandle(position, rotation, handleSize * 0.15f, new Vector3(EditorPrefs.GetFloat("MoveSnapX"), EditorPrefs.GetFloat("MoveSnapY"), EditorPrefs.GetFloat("MoveSnapZ")), Handles.RectangleHandleCap);
        Handles.color = color;
        return position;
    }
#endif
        ...
        public static void CopyMainRoadsData(SerializedProperty original, SerializedProperty copy)
        {
            ...
            copy.FindPropertyRelative("flipUvs").boolValue = original.FindPropertyRelative("flipUvs").boolValue;
            copy.FindPropertyRelative("uvZScale").floatValue = original.FindPropertyRelative("uvZScale").floatValue;
            copy.FindPropertyRelative("uvZOffset").floatValue = original.FindPropertyRelative("uvZOffset").floatValue;
            copy.FindPropertyRelative("startIndex").intValue = original.FindPropertyRelative("startIndex").intValue;
            copy.FindPropertyRelative("endIndex").intValue = original.FindPropertyRelative("endIndex").intValue;
        ...
        public static void SnapPoint(Point[] points, Point point, SerializedObject settings, Point currentPoint)
        {
            if (settings.FindProperty("roadGuidelinesLength").floatValue > 0)
            {
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    {
                        Vector3 neareastPoint = FindNearestPointOnLine(points[i].startGuidelinePoint + points[i].transform.position, points[i].endGuidelinePoint + points[i].transform.position, point.transform.position);

                        if (Vector3.Distance(point.transform.position, neareastPoint) < settings.FindProperty("roadGuidelinesSnapDistance").floatValue)
                        {
                            point.transform.position = neareastPoint;

                            if ((point.transform.GetSiblingIndex() > 0 && Vector3.Angle((point.transform.position - point.transform.parent.GetChild(point.transform.GetSiblingIndex() - 1).position).normalized, points[i].rightLocalControlPointPosition.normalized) > 90) || (point.transform.GetSiblingIndex() == 0 && point.transform.parent.childCount > 1 && Vector3.Angle((point.transform.parent.GetChild(point.transform.GetSiblingIndex() + 1).position - point.transform.position).normalized, points[i].rightLocalControlPointPosition.normalized) > 90))
                            {
                                point.leftLocalControlPointPosition = points[i].rightLocalControlPointPosition;
                                point.rightLocalControlPointPosition = points[i].leftLocalControlPointPosition;
                            }
                            else
                            {
                                point.leftLocalControlPointPosition = points[i].leftLocalControlPointPosition;
                                point.rightLocalControlPointPosition = points[i].rightLocalControlPointPosition;
                            }
                            return;
                        }
                    }
        ...
        public static Vector3 GetLineIntersection(Vector3 point1, Vector3 direction1, Vector3 point2, Vector3 direction2, float distance1 = float.MaxValue)
        {
            ...
            if (Mathf.Abs(planarFactor) < 0.01f && crossVector1and2.sqrMagnitude > 0.01f)
            {
                ...
                if (AlmostEqual(point2 + distance2 * direction2, point, 0.001f) && AlmostEqual(point1 + distance * direction1, point, 0.001f) && distance < pointDistance * 3 && distance2 < pointDistance * 3 && distance < distance1)
                {
        ...
        public static void DisplayControls(ref bool folded, bool showIntersectionControls)
        {
            folded = EditorGUILayout.Foldout(folded, "Show Controls");

            if (folded)
            {
                GUILayout.Label("Create point at the end: Shift + Left click");
                GUILayout.Label("Create point at the start: Shift + CTRL + Left click");
                GUILayout.Label("Move point: Left click + Drag alternatively use the movement handles");
                GUILayout.Label("Move control point separately: A + Drag the movement handles");
                GUILayout.Label("Insert point: Shift + Alt + Left click");
                GUILayout.Label("Remove point: Shift + Right click");
                GUILayout.Label("Lock Y-position for control point (Y-position will always be the same as it's main point): P + Move point");
                GUILayout.Label("Create straight segment: G + Left Click");

                if (showIntersectionControls)
                {
                    GUILayout.Label("");
                    GUILayout.Label("Intersection", EditorStyles.boldLabel);
                    GUILayout.Label("Create intersection: Move two start / end points for two different roads onto eachover");
                    GUILayout.Label("Create T-intersection: Move start / end point to a middle point of a different road");
                    GUILayout.Label("Connect to intersection: Move start / end point on top of an intersection");
                    GUILayout.Label("Remove intersection: Disconnect all connections except one and it will be removed automatically");
                    GUILayout.Label("Move point while keeping intersection connection: Left click + I + Drag, alternatively use the movement handles");
                    GUILayout.Label("Create point at the end: Shift + Left click");
                }
        ...
        public static void AddTag(string name)
        {
            ...
            {
                SerializedProperty tag = layersProperty.GetArrayElementAtIndex(i);

                if (tag.stringValue.Equals(name))
                {
                    return;
                }
            }
            ...
            SerializedProperty newTag = layersProperty.GetArrayElementAtIndex(index);
